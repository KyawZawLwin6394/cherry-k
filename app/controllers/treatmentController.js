...
exports.listAllTreatments = async (req, res) => {
  ...
  try {
    ...
    skip = +skip || 0;
    let query = { isDeleted: false, sellEndFlag: false },
      regexKeyword;
...
exports.getTreatment = async (req, res) => {
  const result = await Treatment.find({ _id: req.params.id, isDeleted: false, sellEndFlag: false }).populate('relatedDoctor').populate('relatedTherapist').populate('relatedPatient').populate('machine.item_id').populate('procedureAccessory.item_id').populate('medicineLists.item_id').populate('procedureMedicine.item_id').populate('treatmentName')
  if (!result)
