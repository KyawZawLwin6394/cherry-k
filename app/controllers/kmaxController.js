...
exports.listAllKmaxVouchers = async (req, res) => {
    ...
    try {
        ...
        console.log(query)
        let result = await KmaxVoucher.find(query).populate('relatedDoctor relatedTreatment secondAccount relatedAppointment relatedPatient relatedBank relatedCash paymentType relatedBranch createdBy relatedAccounting payment medicineSale.item_id procedureSale.item_id accessorySale.item_id')
...
exports.createKmaxVoucher = async (req, res, next) => {
    ...
    try {
        ...
        //prepare CUS-ID
        const latestDocument = await KmaxVoucher.find({}, { seq: 1 }).sort({ _id: -1 }).limit(1).exec();
        if (latestDocument.length == 0) data = { ...data, seq: 1, voucherCode: "KVC-1" } // if seq is undefined set initial patientID and seq
...
exports.getwithExactDate = async (req, res) => {
    try {
        ...
        let result = await KmaxVoucher.find({ createdAt: { $gte: startDate, $lt: endDate } }).populate('relatedPatient relatedTransaction relatedCash').populate('relatedAppointment').populate('medicineItems.item_id').populate('relatedTreatment').populate('createdBy')
...
exports.KmaxVoucherFilter = async (req, res) => {
    ...
    try {
        ...
        if (!bankID) {
            ...
            const CashNames = cashResult.reduce((result, { relatedCash, paidAmount }) => {
                if (relatedCash) {
            ...
            const CashTotal = cashResult.reduce((total, sale) => total + (sale.paidAmount || 0), 0);
            response.data = { ...response.data, CashList: cashResult, CashNames: CashNames, CashTotal: CashTotal }
        }
        //filter solid beauty
        const BankNames = bankResult.reduce((result, { relatedBank, paidAmount }) => {
            if (relatedBank) {
        ...
        const BankTotal = bankResult.reduce((total, sale) => total + (sale.paidAmount || 0) , 0);
