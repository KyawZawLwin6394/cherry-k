...
exports.listAllMedicineSales = async (req, res) => {
  ...
  try {
    ...
    let result = await MedicineSale.find(query).limit(limit).skip(skip).populate('relatedPatient').populate('relatedAppointment').populate('medicineItems.item_id').populate('');
    console.log(result)
