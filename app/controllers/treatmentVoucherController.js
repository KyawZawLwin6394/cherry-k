...
exports.combineMedicineSale = async (req, res) => {
  ...
  const patientUpdate = await Patient.findOneAndUpdate(
    ...
    {
      $inc: {
        conditionAmount: req.body.msPaidAmount,
...
exports.listAllTreatmentVouchers = async (req, res) => {
  ...
  try {
    ...
    let result = await TreatmentVoucher.find(query).populate(
...
exports.getTreatmentVoucherWithTreatmentID = async (req, res) => {
  ...
  const result = await TreatmentVoucher.find(query).populate(
    'createdBy relatedTreatment relatedAppointment relatedPatient'
  )
  if (!result)
...
exports.getTreatmentVoucher = async (req, res) => {
  ...
  const result = await TreatmentVoucher.find(query).populate(
    'createdBy relatedTreatment relatedAppointment relatedPatient relatedTreatmentSelection medicineItems.item_id multiTreatment.item_id relatedPackage relatedPackageSelection'
  )
  if (!result)
    return res.status(500).json({ error: true, message: 'No Record Found' })
  return res.status(200).send({ success: true, data: result })
...
exports.TreatmentVoucherFilter = async (req, res) => {
  ...
  try {
    ...
    if (relatedDoctor) query.relatedDoctor = relatedDoctor
    if (relatedBranch) query.relatedBranch = relatedBranch
    let bankResult = await TreatmentVoucher.find(query)
      .populate(
    ...
    if (!bankID) {
      ...
      let cashResult = await TreatmentVoucher.find(query2)
        .populate(
          'medicineItems.item_id multiTreatment.item_id relatedTreatment secondAccount relatedBranch relatedDoctor relatedBank relatedCash relatedPatient relatedTreatmentSelection relatedAccounting payment createdBy'
        )
        .populate({
    ...
    const BankNames = bankResult.reduce(
      (
        result,
        { relatedBank, paidAmount, msPaidAmount, totalPaidAmount, psPaidAmount }
      ) => {
        if (relatedBank) {
