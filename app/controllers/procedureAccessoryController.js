...
exports.listAllProcedureAccessorys = async (req, res) => {
  ...
  try {
    ...
    let result = await ProcedureAccessory.find(query).limit(limit).skip(skip).populate('relatedAccounting');
...
exports.getProcedureAccessory = async (req, res) => {
  const result = await ProcedureAccessory.find({ _id: req.params.id,isDeleted:false }).populate('relatedAccounting')
  if (!result)
...
exports.searchProcedureAccessories = async (req, res, next) => {
  try {
    const result = await ProcedureAccessory.find({ $text: { $search: req.body.search } })
