...
exports.listAllAccountingLists = async (req, res) => {
    ...
    try {
        ...
        let result = await AccountingList.find(query).limit(limit).skip(skip).populate('relatedDoctor').populate('relatedTherapist').populate('relatedPatient').populate('procedureMedicine.item_id');
        console.log(result)
...
exports.getAccountingList = async (req, res) => {
    const result = await AccountingList.find({ _id: req.params.id, isDeleted: false }).populate('relatedDoctor').populate('relatedTherapist').populate('relatedPatient').populate('procedureMedicine.item_id')
    if (!result)
